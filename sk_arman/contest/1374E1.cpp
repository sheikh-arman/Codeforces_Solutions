...
//    /freopen("1output.txt","w",stdout);
    ...
    for(ll test=1; test<=tcase; test++)
    {
        ...
        VST(V2);
        ll ans=0;
        ll point1=min(k,(ll)V1.size());
        ll point2=min(k,(ll)V2.size());
        ll point3=0;
        ll siz=Vb.size();
        if(point1+siz<k||point2+siz<k)
        {
            cout<<"-1\n";
            continue;
        }
        ...
        while(k)
        {
            if(point3<siz&&point1>=0&&point2>=0)
            {
                if(V1[point1]+V2[point2]>Vb[point3])
                {
                    ans+=Vb[point3];
                    point3++;
                    k--;
